...
// Returns the right stored Stripe price id for a given user.
export function stripePriceIdForUser(
  ...
  const isSubscriber = profile?.subscription_status === "active";
  if (isSubscriber && cohort.stripe_price_id_subscriber) {
    return cohort.stripe_price_id_subscriber;
  }
  return cohort.stripe_price_id_standard;
}
