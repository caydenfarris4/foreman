...
export default async function AdminCohortDetail({
  ...
  // Waitlist + office-hours bookings (phase 9 visibility).
  const { data: wlRows } = await supabase
    .from("cohort_waitlist")
    .select("email, name, created_at")
    .eq("cohort_id", id)
    .order("created_at", { ascending: true });
  const waitlist = (wlRows ?? []) as {
    email: string;
    name: string | null;
    created_at: string;
  }[];
  const { count: officeHoursCount } = await supabase
  ...
  return (
    <div className="space-y-8">
      ...
      {waitlist.length > 0 || (officeHoursCount ?? 0) > 0 ? (
        <div className="rounded-lg border border-rule bg-chalk p-4">
          <p className="type-cap text-graphite">
            WAITLIST ({waitlist.length}) · OFFICE HOURS BOOKED (
            {officeHoursCount ?? 0})
          </p>
          {waitlist.length > 0 ? (
            <ul className="mt-2 space-y-1">
              {waitlist.map((w) => (
                <li key={w.email} className="type-body-sm text-ink2">
                  {w.name ? `${w.name} · ` : ""}
                  {w.email}
