...
async function signup(formData: FormData) {
  ...
  // Build the confirmation-email landing URL with `next` baked in so the
  // user lands back where they started (e.g. /cohorts/X/apply) instead
  // of the default /onboarding.
  const appUrl =
  ...
  // Two outcomes:
  //   1. Email confirmation OFF: session created -> jump to next (or
  //      onboarding if none).
  //   2. Email confirmation ON: signUp succeeds but no session yet —
  //      show the "check your email" state, carrying `next` so the
  //      confirmation link will land them where they came from.
